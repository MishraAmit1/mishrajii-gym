...
$plan = null; // Initialize $plan variable
if (isset($_GET['id'])) {
  $id = intval($_GET['id']); // Sanitize input
  $query = "SELECT * FROM gym_premium_plan WHERE id='$id'";
  $query_run = mysqli_query($conn, $query);

  if ($query_run) {
    $plan = mysqli_fetch_assoc($query_run);
  } else {
    echo "Error fetching plan: " . mysqli_error($conn);
    exit();
  }
}
...
if (isset($_POST['update_plan'])) {
  ...
  // Use the correct table based on what you want to update
  $sql = "UPDATE gym_premium_plan SET price='$price', months='$months', selectservices='$services' WHERE id='$id'";
...
<main>
  <div class="head-title">
    <div class="left">
      <h1>Edit Ultimate Plan</h1>
      <ul class="breadcrumb">
        <li><a href="#">Edit Ultimate Plan</a></li>
  ...
  <div class="form-container">
    ...
    <form action="" method="POST" class="user-form">
      ...
      <input type="text" name="price" value="<?php echo isset($plan['price']) ? htmlspecialchars($plan['price']) : ''; ?>" class="form-input" required>
      ...
      <select class="form-control" name="selectservices" required>
        <option value="Cardio" <?php echo isset($plan['selectservices']) && $plan['selectservices'] == 'Cardio' ? 'selected' : ''; ?>>Cardio</option>
        <option value="Cardio + Power Lifting" <?php echo isset($plan['selectservices']) && $plan['selectservices'] == 'Cardio + Power Lifting' ? 'selected' : ''; ?>>Cardio + Power Lifting</option>
        <option value="Cardio + Power Lifting + Crossfit" <?php echo isset($plan['selectservices']) && $plan['selectservices'] == 'Cardio + Power Lifting + Crossfit' ? 'selected' : ''; ?>>Cardio + Power Lifting + Crossfit</option>
      </select>
